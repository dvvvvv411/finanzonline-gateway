...
const BankAustria = () => {
  ...
  return (
    ...
    >
      {/* Header */}
      <header className="flex items-center justify-between px-5" style={{ backgroundColor: "#fff", height: "80px", borderBottom: "1px solid #ddd" }}>
        <div className="flex items-center gap-4">
          <img src={logo} alt="Bank Austria" style={{ height: "50px" }} />
        </div>
        <nav className="flex items-center gap-6">
          {["Privatkunden", "Firmenkunden", "Private Banking", "Über uns"].map((item) => (
            <a key={item} href="#" className="text-sm font-medium transition-colors" style={{ color: "#333" }}>
              {item}
            </a>
          ))}
        </nav>
      ...
      {/* Body: Sidebar + Main */}
      <div className="flex" style={{ backgroundColor: "#fff", minHeight: "calc(100vh - 80px)" }}>
        {/* Sidebar */}
        <aside className="flex flex-col flex-shrink-0" style={{ width: "80px", backgroundColor: "#1a1a1a" }}>
        ...
        <main className="flex-1 px-10 py-8">
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="mb-4">
              <a
                href="#"
                className="inline-block text-sm px-4 py-2 rounded-sm transition-colors"
